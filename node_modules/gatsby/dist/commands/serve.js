...
var _getConfigFile = _interopRequireDefault(require("../bootstrap/get-config-file"));

var _preferDefault = _interopRequireDefault(require("../bootstrap/prefer-default"));
...
module.exports = async program => {
  ...
  const {
    configModule
  } = await (0, _getConfigFile.default)(program.directory, `gatsby-config`);
  const config = (0, _preferDefault.default)(configModule);
  const {
  ...
  const startListening = () => {
    app.listen(port, host, () => {
      ...
      _reporter.default.info(`gatsby serve running at: ${_chalk.default.bold(openUrlString)}`);

      if (open) {
        _reporter.default.info(`Opening browser...`);

        Promise.resolve((0, _betterOpn.default)(openUrlString)).catch(() => _reporter.default.warn(`Browser not opened because no browser was found`));
      }
    });
  };
